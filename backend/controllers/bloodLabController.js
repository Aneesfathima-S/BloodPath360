...
export const updateBloodRequestStatus = async (req, res) => {
  try {
    ...
    if (action === "accept") {
      ...
      console.log("Lab stock:", labStock ? `${labStock.quantity} units` : "Not found");
      ...
      // 2. Deduct from lab stock
      labStock.quantity -= request.units;
      console.log("Lab stock after deduction:", labStock.quantity);
      
      if (labStock.quantity === 0) {
        await Blood.findByIdAndDelete(labStock._id);
        console.log("Lab stock deleted (reached 0)");
      } else {
        await labStock.save();
        console.log("Lab stock saved");
      }

      // 3. Add to hospital stock
      let hospitalStock = await Blood.findOne({
        hospital: request.hospitalId._id,
        bloodGroup: request.bloodType
      });

      console.log("Hospital stock found:", hospitalStock ? `${hospitalStock.quantity} units` : "Creating new");

      const expiryDate = new Date();
      expiryDate.setDate(expiryDate.getDate() + 42);

      if (hospitalStock) {
        // Update existing hospital stock
        hospitalStock.quantity += request.units;
        hospitalStock.expiryDate = expiryDate;
        await hospitalStock.save();
        console.log("Hospital stock updated:", hospitalStock.quantity);
      } else {
        // Create new hospital stock - CRITICAL: Only set hospital field
        const newStock = await Blood.create({
          bloodGroup: request.bloodType,
          quantity: request.units,
          expiryDate,
          hospital: request.hospitalId._id,
          status: "available"
          // Do NOT include bloodLab field
        });
        console.log("New hospital stock created:", newStock);
      }

      // 4. Add to lab history
      await Facility.findByIdAndUpdate(labId, {
        $push: {
          history: {
            eventType: "Stock Update",
